...
<div style="background-color:#d4d4d4; padding:40px 10vw;">
  <div style="text-align:center; background-color:#f8f8f8;">
    <h2 style="font-size:1.8em; background-color: #dff0d8; color: #3c763d; padding:25px;"><?=$news->name?></h2>
    <?php if($news->image !== null && $news->image !== '') : ?>
      <img src="<?= $message->embed($image); ?>" style="margin: 20px auto; width: 60%;">
    <?php endif; ?>
    <h3 style="font-size:1.5em; padding:10px 20%;"><?=$news->content?></h3>
    ...
    <a href="<?= Yii::$app->urlManager->createAbsoluteUrl(['auth/unsubscribe', 'email' => Html::encode($email)]);?>">
      <button style="padding:10px;font-size:1em; margin-bottom:10px;
